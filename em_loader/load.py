...
def load_scisrs_dataset(base_path, csv_path, bounding_box_format):
    ...
    def format_data(image, bounding_boxes, class_ids):
        ...
        return image, bounding_box.convert_format(bounding_boxes, source='yxyx', target=bounding_box_format, images=image)
...
def load(
     bounding_box_format, batch_size=None, split="train", data_dir=None, download=True, verbosity=1, with_info=True
):
    ...
    ds, ds_info = load_scisrs_dataset(data_path, csv_path, bounding_box_format=bounding_box_format)
